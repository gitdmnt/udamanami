...
fn val_as_list(val: &EvalResult) -> Option<Vec<Box<EvalResult>>> {
    match val {
        ...
        EvalResult::SVal(s) => Some(
            s.chars()
                .map(|c| Box::new(EvalResult::SVal(c.to_string())))
                .collect(),
        ),
        _ => None,
    }
...
fn list_free_var(expr: &Expr) -> HashSet<String> {
    let result = match expr {
        ...
        Expr::List(l) => l
            .iter()
            .map(|e| list_free_var(e))
            .fold(HashSet::new(), |acc, x| acc.union(&x).cloned().collect()),
        Expr::Object(o) => o
            .iter()
            .map(|(k, v)| list_free_var(v))
            .fold(HashSet::new(), |acc, x| acc.union(&x).cloned().collect()),
...
fn eval_expr_ctx(
    ...
    let result = match expr {
        ...
        Expr::Const(s) => match local_context.get(s) {
            Some(result) => Ok((result.clone(), step)),
            None => match global_context.get(s) {
                Some(result) => Ok((result.clone(), step)),
            ...
            match shortsurcuit {
                ...
                None => {
                    ...
                    match op {
                        ...
                        ExprOp2::Pow => {
                            val_numop2_f(expr, step, &val1, &val2, |f1, f2| f1.powf(f2))
                        }
                ...
                Box::new(Expr::Apply(
                    ...
                    vec![Box::new(Expr::Lambda(
                        ...
                        Box::new(Expr::Apply(
                            ...
                            vec![Box::new(Expr::Const("_y".to_string()))],
                        )),
                    ))],
                )),
            );
