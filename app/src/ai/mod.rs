...
impl ManamiAi {
    ...
    fn tool_definitions(&self) -> Vec<ToolDefinition> {
        vec![
            tools::watch::Watch::def(),
            tools::web_search_wikipedia::WebSearchWikipedia::def(),
            tools::web_fetch_wikipedia::WebFetchWikipedia::def(),
        ]
    }

    async fn dispatch_tool(&self, name: &str, args: serde_json::Value) -> Result<String, String> {
        match name {
            "watch" => tools::watch::Watch::call(args).await,
            "web_search_wikipedia" => {
                tools::web_search_wikipedia::WebSearchWikipedia::call(args).await
            }
            "web_fetch_wikipedia" => {
                tools::web_fetch_wikipedia::WebFetchWikipedia::call(args).await
            }
